 ...
 * 周期和持续时间
 * <p>
 * 当您编写代码来指定一段时间时，请使用最符合您需要的类或方法：
 * Duration 类， Period 类或 ChronoUnit.between 方法。
 * Duration 使用基于时间的值（秒，毫微秒）的时间量。Period 使用基于日期的值（年，月，日）。
 *
...
class PeriodAndDurationTest {
  ...
  /**
   * 在测量机器时间的情况下，Duration 最合适，例如使用 Instant 对象的代码。
   * Duration 对象以秒或纳秒度量， 不使用基于 Date 的结构，如年、月和日，尽管类提供了转换为天数、小时和分钟的方法。
   * 一个 Duration 可以有一个负值， 如果它是在开始点之前发生的端点创建的。
   */
  ...
  /**
   * 要用基于日期的值（年、月、日）来定义大量的时间，使用周期类。
   * 周期类提供了各种 get 方法， 例如 getMonths， getDays 和 getYears，这样您就可以从周期中提取出时间的数量。
   */

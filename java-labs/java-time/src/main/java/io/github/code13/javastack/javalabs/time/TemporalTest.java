...
class TemporalTest {
   ...
   * 这些接口的目的是在最低级别使用。典型的应用程序代码应该以具体类型的形式声明变量和参数， 比如 LocalDate 或 ZonedDateTime. 而不是 Temporal 接口。这与声明 String 类型的变量完全相同，而不是 CharSequence 类型。
   */

  /**
   * Temporal 接口提供了访问 temporal - based 对象的框架；并通过基于时间的类，如实现 Instant, LocalDateTime, and ZonedDateTime. 该接口提供了添加或减少时间单位的方法，使得基于时间的算术在各种日期和时间类中变得简单和一致。
   * TemporalAccessor 接口提供的只读版本 Temporal 接口。
   * <p>
   * Temporal and TemporalAccessor 对象是用字段来定义的，如TemporalField 接口;字段的单位由 TemporalUnit 接口指定
   * Temporal 接口中的基于算术的方法需要使用 TemporalAmount 值定义的参数
   */

  /**
   * 实现 TemporalField 接口的 ChronoField 枚举提供了一组用于访问日期和时间值的常量。
   * 一些示例是 CLOCK_HOUR_OF_DAY，NANO_OF_DAY 和 DAY_OF_YEAR。
   * 此枚举可用于表示时间的概念方面， 例如一年的第三周，一天的第 11 个小时或本月的第一个星期一。
   * 当你遇到位置类型的 Temporal 时， 可以使用 TemporalAccessor.isSupported（TemporalField） 方法来确定时态是否支持特定的字段。
   * 下面这行代码返回 false，表明 LocalDate 不支持 ChronoField.CLOCK_HOUR_OF_DAY 字段的单位由。
   */
  @DisplayName("java.")

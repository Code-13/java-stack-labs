 ...
 * FairLockRunner.
 *
 * <p>Redis based distributed reentrant fair Lock object for Java implements Lock interface.
 *
 * <p>Fair lock guarantees that threads will acquire it in is same order they requested it. All
 * waiting threads are queued and if some thread has died then Redisson waits its return for 5
 * seconds. For example, if 5 threads are died for some reason then delay will be 25 seconds.
 *
 ...
 */
@DisplayName("Fair Lock")
class FairLockRunner {
  ...
  void lock1() {
    RLock lock = redissonClient.getFairLock("Lock1");
    lock.lock();
    try {
      // do something
    } finally {
  ...
  void lock3() throws InterruptedException {
    RLock lock = redissonClient.getFairLock("Lock3");
